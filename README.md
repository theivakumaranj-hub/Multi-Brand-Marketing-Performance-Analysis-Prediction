...
* `train_models.py` — The core machine learning pipeline. This script cleans the raw data, handles one-hot encoding, trains the Scikit-Learn models, and exports them.
...
* `purplle_campaign_data_with_nulls.csv` — Raw marketing performance metrics from Purplle campaign runs, including uncleaned entries and null records.
* `tira_campaign_data_with_nulls.csv` — Raw campaign platform data sourced from Tira, requiring extensive data cleansing and imputation.
* `cleaned_campaign_data.csv` — The final, consolidated, and fully cleaned historical dataset generated after handling null values, merging the three platform sources, and processing features for EDA and Machine Learning model training.

**4. Documentation & Setup**
* `requirements.txt` — List of required Python dependencies (`streamlit`, `pandas`, `scikit-learn`, etc.).
...
`bash
https://github.com/theivakumaranj-hub/Multi-Brand-Marketing-Performance-Analysis-Prediction/tree/main
`
